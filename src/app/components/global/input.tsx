...
const inputVariants = cva(
  'block w-full text-white rounded-lg border border-gray-400 bg-primary p-2.5 transition-all duration-300 focus:border-white focus:ring-white',
  {
    variants: {
      variant: {
        round: 'rounded-full',
...
export function Input({
  ...
  return (
    <div className={cn('block', className)}>
      <label
        htmlFor={props.name}
        className={cn('mb-2 block text-lg font-medium text-white')}
      >
        {label}
      </label>
...
export function DateInput({
  ...
  return (
    <div>
      <label
        htmlFor={name}
        className="drop-shadow-glow mb-1 block text-lg md:text-2xl"
      >
        {label}
      </label>
      <input
